...
| `mcp.server` | program level only: how a client should RUN the server — `env`, `toolTimeoutMs`, `startupTimeoutMs`. Declared once in these units and projected into each client's own spelling by `mcp install`; a client with no equivalent receives nothing for it |
